...
const Chat = () => {
	...
	return (
		...
		>
			...
			<View className='flex-1 w-full relative'>
				<FlatList
					...
					data={
						sendLoading
							? [
									{ id: 'loading', text: '', type: 'loading', isMe: false },
									...messages,
								]
							: messages
					}
					renderItem={({ item }) => (
						<View
							className={cn(
								'w-full flex-row my-3 px-6 items-center',
								item.isMe ? 'justify-end' : 'justify-start'
							)}
						>
							<View
								className={cn(
									'max-w-[80%] rounded-2xl px-6 py-3 border border-[#000] transition-all duration-300 ease-in-out min-h-[40px] flex items-center justify-center',
									item.isMe ? 'bg-orange-100' : 'bg-white'
								)}
							>
								{item.type === 'loading' ? (
									<View className='flex-row items-center'>
										...
										<View
											className='w-2 h-2 bg-orange-500 rounded-full mr-1 animate-pulse'
			...
			{/* 聊天输入框 */}
			<View
				className='w-full bg-orange-200 p-4 rounded-t-xl flex-row items-center transition-all duration-300 ease-in-out'
			...
			>
				...
				/>
				<TouchableOpacity
					className='h-10 w-10 bg-white rounded-3xl px-0 py-0 border border-[#000] ml-4 items-center justify-center'
					onPress={handleSend}
				>
